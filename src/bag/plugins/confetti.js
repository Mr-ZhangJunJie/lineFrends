...
window.Confetti = function () {
  function Confetti(param) {
    ...
    this.canvas.style.cssText = ["display: block", "position: absolute", "top: 0", "left: 0", "z-index: 50", "pointer-events: none"].join(";");
    ...
    this.requestAnimationFrame1 = requestAnimationFrame(this.render);
  }
  ...
  Confetti.prototype.build = function build() {
    for (var i = 0; i < this.length; ++i) {
      ...
      canvas.position = {
        ...
        initY: - canvas.height * .6 - Math.random() * this.yRange
      };
  ...
  Confetti.prototype.render = function render(now) {
    ...
    this.requestAnimationFrame2 = requestAnimationFrame(this.render);
  }

  Confetti.prototype.cancel = function() {
    window.cancelAnimationFrame(this.requestAnimationFrame1)
    window.cancelAnimationFrame(this.requestAnimationFrame2)
    this.sprites = []
    this.canvas = null
    this.ctx = null
    this.parent.removeChild(document.getElementById('confettiCanvas'))
  }
  ...
  _createClass(Confetti, null, [{
    ...
    get: function get() {
      return {
        SPRITE_WIDTH: 9,//纸屑宽
        SPRITE_HEIGHT: 16,//纸屑高
        PAPER_LENGTH: 100,//纸屑数量
        DURATION: 8000,
        ROTATION_RATE: 50,
        ...
        // 彩条颜色
        COLORS: ["#fff", "#fff", "#fff", "#fff", "#5C6BC0", "#42A5F5", "#29B6F6", "#26C6DA", "#26A69A", "#66BB6A", "#9CCC65", "#D4E157", "#FFEE58", "#FFCA28", "#FFA726", "#FF7043", "#8D6E63", "#BDBDBD", "#78909C"]
      };
    }
  }]);
